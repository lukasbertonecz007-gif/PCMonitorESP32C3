...
constexpr size_t SERIAL_BUFFER_SIZE = 860;
...
void drawDynamic(bool force) {
  ...
  bool staleChanged = force || wokeUp || !renderCache.initialized || renderCache.stale != stale;

  if (!renderCache.initialized || force || wokeUp) {
    drawStaticLayout();
    resetHistory();
    renderCache = RenderCache();
    for (int i = 0; i < MAX_THREAD_TILES; i++) {
  ...
  updateSmoothedValues(stale, force || staleChanged);
  tft.startWrite();
  updateRows(force || staleChanged);
  updateCpuInfoPanel(stale, force || staleChanged);
  updatePowerPanel(force || staleChanged);
  updateNetworkGraph(stale, force || staleChanged);
  tft.endWrite();
...
void drawBottomSkeleton() {
  ...
  drawCenteredText(POWER_PANEL_X + 2, BOTTOM_Y + 2, POWER_PANEL_W - 4, 10, "TOTAL POWER", 1, COLOR_TEXT_DIM, COLOR_PANEL);
}

void updateSmoothedValues(bool stale, bool force) {
  smoothState.cpuUsage = smoothValue(smoothState.cpuUsage, stale ? -1.0f : telemetry.cpuUsage, 0.24f, force);
  smoothState.cpuTemp = smoothValue(smoothState.cpuTemp, stale ? -1.0f : telemetry.cpuTemp, 0.20f, force);

  smoothState.gpuUsage = smoothValue(smoothState.gpuUsage, stale ? -1.0f : telemetry.gpuUsage, 0.24f, force);
  smoothState.gpuTemp = smoothValue(smoothState.gpuTemp, stale ? -1.0f : telemetry.gpuTemp, 0.20f, force);

  smoothState.ramUsage = smoothValue(smoothState.ramUsage, stale ? -1.0f : telemetry.ramUsage, 0.22f, force);
  smoothState.ramUsedGb = smoothValue(smoothState.ramUsedGb, stale ? -1.0f : telemetry.ramUsedGb, 0.18f, force);
  smoothState.ramTotalGb = smoothValue(smoothState.ramTotalGb, stale ? -1.0f : telemetry.ramTotalGb, 0.16f, force);

  smoothState.netDownMBps = smoothValue(smoothState.netDownMBps, stale ? -1.0f : telemetry.netDownMBps, 0.26f, force);
  smoothState.netUpMBps = smoothValue(smoothState.netUpMBps, stale ? -1.0f : telemetry.netUpMBps, 0.26f, force);

  smoothState.totalPowerW = smoothValue(smoothState.totalPowerW, stale ? -1.0f : telemetry.totalPowerW, 0.20f, force);
}
...
void drawDiskPanel(bool stale, bool force) {
  ...
  if (stale || diskSeriesCount <= 0 || historyCount < 2) {
    if (useSprite) {
  ...
  drawDiskLegend(legendPos > 0 ? legend : "DISKS", false);
}
